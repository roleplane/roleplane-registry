...
3. **Injection lint** — the fetched content (the skill file, or a team's `config.yaml` plus its agent/skill markdown one directory level deep) is scanned against the versioned rules spec in `src/injection-rules.json` (tool-invocation phrasing, embedded URLs, webhook endpoints, exfiltration patterns, instruction overrides). Findings are **warnings only**: they surface on the PR for the maintainer to review with eyes open, but never fail the job. A screening aid, not a security boundary.
